          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {mangoes.slice(0, 4).map((mango) => (
              <div key={mango.id} className="reveal-item group bg-white rounded-3xl p-5 md:p-6 luxury-shadow flex flex-col md:flex-row items-center border border-brand-gold/10 hover:border-brand-gold/20 transition-all duration-500 gap-6">
                <Link to={`/product/${mango.id}`} className="w-full md:w-1/2 aspect-square overflow-hidden rounded-2xl shadow-sm">
                ...
                <div className="w-full md:w-1/2">
                   ...
                   <Link to={`/product/${mango.id}`} className="text-lg md:text-xl font-bold text-brand-green mb-2 font-serif leading-tight group-hover:text-brand-gold transition-colors block">{mango.name}</Link>
